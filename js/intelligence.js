...
async function session(){if(!window.supabaseClient)return null;const{data:{session}}=await window.supabaseClient.auth.getSession();return session}
...
async function loadCustomer(){
  ...
  const{data,error}=await window.supabaseClient.from('customers').select('id,plan').eq('auth_user_id',s.user.id).maybeSingle();
...
async function updateUsage(){
  ...
  const{count}=await window.supabaseClient.from('ai_requests').select('*',{count:'exact',head:true}).eq('customer_id',currentCustomerId).gte('created_at',start);
...
async function loadHistory(){
  ...
  const{data,error}=await window.supabaseClient.from('ai_conversations').select('id,title,agent_type,updated_at').eq('customer_id',currentCustomerId).order('updated_at',{ascending:false}).limit(15);
...
async function openConversation(item,button){
  ...
  const{data,error}=await window.supabaseClient.from('ai_requests').select('prompt,answer,created_at').eq('customer_id',currentCustomerId).eq('conversation_id',item.id).order('created_at',{ascending:true});
